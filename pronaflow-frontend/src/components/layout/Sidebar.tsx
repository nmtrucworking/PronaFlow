...
const NAVIGATION_DATA: SectionData[] = [
  {
    ...
    items: [
      ...
      { id: 'settings-over', label: 'Over-Setting', icon: Activity, path: '/settings-over' },
    ]
  },
  {
    id: 'favorites',
...
export default function App({ 
  ...
  return (
    <div className="flex h-screen bg-slate-100 overflow-hidden font-sans text-slate-900">
      <aside 
        ref={sidebarRef}
        className={`relative flex flex-col bg-slate-50 border-r border-slate-200 transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] z-30 ${
      ...
      >
        ...
        <div className="p-3 pt-4">
          <div 
            ...
            className={`flex items-center gap-2 bg-white border border-slate-200 rounded-lg h-9 px-2.5 transition-all group ${
              isCollapsed ? 'justify-center border-transparent bg-transparent cursor-pointer' : 'hover:border-slate-300'
            }`}
          >
            ...
            <div key={section.id} className="space-y-1">
              ...
              <div 
                ...
                className={`flex items-center justify-between px-2 py-1.5 rounded-md transition-colors ${
                  isCollapsed ? 'justify-center cursor-default' : 'cursor-pointer hover:bg-slate-200/30 group'
                }`}
              >
                {!isCollapsed ? (
                  <>
...
function NavItem({ 
  ...
  return (
    <button 
      onClick={handleClick}
      className={`group relative flex items-center h-10 px-3 rounded-lg transition-all duration-200 text-sm font-medium w-full ${
        isActive 
