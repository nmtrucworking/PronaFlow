...
type ProjectStatus = 'DRAFT' | 'PLANNING' | 'EXECUTION' | 'MONITORING' | 'COMPLETED' | 'ON_HOLD';
type Priority = 'CRITICAL' | 'HIGH' | 'MEDIUM' | 'LOW';
type TaskType = 'STORY' | 'TASK' | 'BUG';
type TaskStatus = 'TODO' | 'IN_PROGRESS' | 'REVIEW' | 'DONE';
type FileType = 'PDF' | 'DOC' | 'IMG' | 'XLS' | 'OTHER';
...
const MOCK_MEMBERS: Member[] = [
  { id: 'u1', name: 'Nguyễn Văn A', avatar: 'https://ui-avatars.com/api/?name=Nguyen+Van+A&background=0D8ABC&color=fff', role: 'Project Manager', email: 'nguyenvana@company.com' },
  { id: 'u2', name: 'Trần Thị B', avatar: 'https://ui-avatars.com/api/?name=Tran+Thi+B&background=E11D48&color=fff', role: 'Developer', email: 'tranthib@company.com' },
  { id: 'u3', name: 'Lê C', avatar: 'https://ui-avatars.com/api/?name=Le+C&background=059669&color=fff', role: 'Tester', email: 'lec@company.com' },
  { id: 'u4', name: 'Phạm D', avatar: 'https://ui-avatars.com/api/?name=Pham+D&background=7C3AED&color=fff', role: 'Stakeholder', email: 'phamd@company.com' },
];

const MOCK_NOTES: Note[] = [
  { id: 'n1', title: 'Đặc tả yêu cầu kỹ thuật v1.0', excerpt: 'Tài liệu chi tiết về kiến trúc hệ thống và các API endpoints cần thiết...', author: MOCK_MEMBERS[0], updatedAt: '2024-03-10', tags: ['Specs', 'Architecture'] },
  { id: 'n2', title: 'Biên bản họp Kick-off', excerpt: 'Ghi nhận các thống nhất về phạm vi dự án, timeline và các mốc bàn giao...', author: MOCK_MEMBERS[1], updatedAt: '2024-03-01', tags: ['Meeting', 'Planning'] },
];

const MOCK_FILES: ProjectFile[] = [
  { id: 'f1', name: 'SRS_Document_v2.pdf', type: 'PDF', size: '2.4 MB', uploader: MOCK_MEMBERS[0], uploadedAt: '2024-03-12' },
  { id: 'f3', name: 'Budget_Q2_2024.xlsx', type: 'XLS', size: '45 KB', uploader: MOCK_MEMBERS[3], uploadedAt: '2024-03-20' },
];

const MOCK_PROJECT_TASKS: Task[] = [
  { id: 't1', key: 'MKT-101', title: 'Thiết kế Wireframe Trang chủ', status: 'DONE', priority: 'HIGH', assignee: MOCK_MEMBERS[1], startDate: '2024-03-01', dueDate: '2024-03-05', progress: 100, type: 'STORY' },
  { id: 't2', key: 'MKT-102', title: 'Viết nội dung giới thiệu sản phẩm', status: 'IN_PROGRESS', priority: 'MEDIUM', assignee: MOCK_MEMBERS[2], startDate: '2024-03-06', dueDate: '2024-03-10', progress: 60, type: 'TASK', dependencies: ['t1'] },
  { id: 't3', key: 'MKT-103', title: 'Tích hợp cổng thanh toán VNPay', status: 'TODO', priority: 'CRITICAL', assignee: MOCK_MEMBERS[1], startDate: '2024-03-12', dueDate: '2024-03-20', progress: 0, type: 'STORY', dependencies: ['t2'] },
  { id: 't4', key: 'MKT-104', title: 'Lỗi hiển thị banner trên Safari mobile', status: 'REVIEW', priority: 'HIGH', assignee: MOCK_MEMBERS[1], startDate: '2024-03-06', dueDate: '2024-03-08', progress: 90, type: 'BUG' },
];

const MOCK_AGILE_PROJECT: Project = {
  id: 'p1',
  key: 'MKT-24',
  name: 'Chiến dịch Marketing Mùa Hè 2024',
  description: 'Triển khai quảng cáo đa kênh và phát triển landing page cho dòng sản phẩm mới.',
  type: 'AGILE',
  status: 'EXECUTION',
  priority: 'HIGH',
  progress: 45,
  ...
  tags: ['Marketing', 'Q2-2024', 'Digital']
};

const MOCK_WATERFALL_PROJECT: Project = {
  id: 'p2',
  key: 'INFRA-01',
  name: 'Nâng cấp Hạ tầng Server Hà Nội',
  description: 'Di dời trung tâm dữ liệu và nâng cấp phần cứng cho cụm máy chủ phía Bắc.',
  type: 'WATERFALL',
  status: 'EXECUTION',
  priority: 'CRITICAL',
  progress: 30,
  ...
  manager: MOCK_MEMBERS[0],
  members: MOCK_MEMBERS,
  tags: ['Infrastructure', 'Hà Nội', 'Critical']
};

const TASK_STATUS_LABELS: Record<TaskStatus, string> = { TODO: 'Cần làm', IN_PROGRESS: 'Đang làm', REVIEW: 'Đang duyệt', DONE: 'Hoàn thành' };
...
function ProjectPriorityPopover({ currentPriority }: { currentPriority: Priority }) {
    const [isOpen, setIsOpen] = useState(false);
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-500">
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-slate-100">{MOCK_PROJECT_TASKS.map(task => (
                                ...
                                <tr key={m.id} className="hover:bg-slate-50/50 transition-colors">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <img src={m.avatar} className="w-10 h-10 rounded-full ring-2 ring-white shadow-sm" />
                                            <div>
                ...
                <div className="flex items-center gap-8 self-start bg-slate-50 p-4 rounded-2xl border border-slate-100 shadow-sm">
                    <div className="text-right">
                        ...
                        <div className="flex items-center justify-end gap-2 group cursor-pointer">
                            ...
                            <img src={currentProject.manager.avatar} className="w-8 h-8 rounded-full ring-2 ring-white shadow-sm" />
                        </div>
                    </div>
                    ...
                    <div className="text-right">
                        ...
                        <div className="flex items-center justify-end gap-1.5 text-sm font-bold text-slate-800">
                            ...
                            {new Date(currentProject.endDate).toLocaleDateString('vi-VN')}
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        {MOCK_NOTES.map(note => (
                            <div key={note.id} className="group bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-lg hover:border-indigo-200 transition-all cursor-pointer flex flex-col h-full relative">
                                ...
                                <div className="flex items-center justify-between pt-4 border-t border-slate-50 mt-auto">
                                    <div className="flex items-center gap-2"><img src={note.author.avatar} alt={note.author.name} className="w-5 h-5 rounded-full shadow-sm" /><span className="text-xs font-medium text-slate-600">{note.author.name}</span></div>
                    ...
                    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                        <table className="w-full text-sm text-left">
                            ...
                            <tbody className="divide-y divide-slate-100">{MOCK_FILES.map(f => (
                                <tr key={f.id} className="hover:bg-slate-50/50 transition-colors group cursor-pointer"><td className="px-6 py-4 text-center"><FileTypeIcon type={f.type} /></td><td className="px-6 py-4"><div className="font-bold text-slate-800 group-hover:text-indigo-600 transition-colors">{f.name}</div><div className="text-[10px] text-slate-400 uppercase font-bold mt-1 tracking-wider">{f.type} Document</div></td><td className="px-6 py-4 text-center text-slate-500 font-mono text-xs">{f.size}</td><td className="px-6 py-4"><div className="flex items-center gap-2"><img src={f.uploader.avatar} className="w-6 h-6 rounded-full" /><span className="text-xs font-medium text-slate-600">{f.uploader.name}</span></div></td><td className="px-6 py-4 text-right"><button className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-full transition-all"><Download className="w-4 h-4" /></button></td></tr>
                            ))}</tbody>
                        </table>
                    </div>
                </div>
