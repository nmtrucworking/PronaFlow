    ...
    <div className="flex flex-col h-screen max-h-screen bg-slate-50 dark:bg-slate-950 overflow-hidden font-sans select-none transition-all duration-500">
      ...
      <Dialog.Root open={!!selectedItem} onOpenChange={(open) => !open && setSelectedItem(null)}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-xl bg-white dark:bg-slate-900 rounded-lg shadow-[0_20px_50px_-12px_rgba(0,0,0,0.35)] z-[101] border border-slate-200/60 dark:border-slate-800 overflow-hidden animate-in zoom-in-95 duration-200 outline-none">
            {selectedItem && (
              <div className="flex flex-col h-[650px]">
                ...
                <div className="h-24 bg-slate-900 relative shrink-0">
                  ...
                  <Dialog.Close className="absolute right-4 top-4 p-2 bg-white/10 hover:bg-white/15 text-white rounded-md transition-all backdrop-blur-sm">
                    <X size={18} strokeWidth={2.5} aria-hidden="true" />
                    <span className="sr-only">Đóng modal</span>
                  </Dialog.Close>
                  <div className="absolute bottom-3 left-10 flex items-center gap-2 px-2.5 py-1 bg-emerald-500/15 text-emerald-300 rounded-md text-[9px] font-black uppercase tracking-widest backdrop-blur-sm border border-emerald-500/30">
                ...
                <div className="px-10 -mt-10 flex-1 overflow-y-auto custom-scrollbar-y pb-10">
                  <div className="flex items-end gap-6 mb-10 relative">
                    <div className={`w-24 h-24 rounded-lg flex items-center justify-center border-[3px] border-white dark:border-slate-900 shadow-xl ${TYPE_CONFIG[selectedItem.type].bg}`}>
                  ...
                  <div className="grid grid-cols-2 gap-3.5 mb-10 animate-in fade-in slide-in-from-bottom-2 duration-400">
                  ...
                  <div className="space-y-8">
                    <section>
                      ...
                      <div className="p-4 bg-slate-50 dark:bg-slate-800/40 rounded-lg text-xs font-medium text-slate-600 dark:text-slate-300 leading-relaxed border border-slate-100 dark:border-slate-800 italic">
                        "{selectedItem.reason}"
                      </div>
                    </section>
                 ...
                 <div className="px-10 py-4 border-t border-slate-100 dark:border-slate-800 shrink-0 bg-slate-50/50 dark:bg-slate-800/30 flex gap-3">
                   <button className="flex-1 py-2.5 bg-blue-600 text-white rounded-lg text-[11px] font-black shadow-lg hover:opacity-95 active:scale-95 transition-all uppercase tracking-widest flex items-center justify-center gap-2">
                      <RotateCcw size={14} strokeWidth={3} /> KHÔI PHỤC DỮ LIỆU
                   </button>
                   <button className="px-5 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-red-500 hover:bg-red-50 transition-all active:scale-90">
                      <Trash2 size={18} />
                   </button>
                </div>
              </div>
